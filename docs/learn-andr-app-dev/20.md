...
最后，在我们进入本书其余部分的应用程序开发之前，让我们确保我们的 Android 开发环境得到了优化配置。这是在 Eclipse 中通过**窗口![image](img/arrow.jpg)首选项**菜单序列完成的，它会打开 Eclipse 首选项对话框。在对话框的左窗格中，您会看到一个左侧带有箭头的 Android 部分，它打开了一个子部分，显示所有 Android 首选项配置区域。选择顶部的主窗口(单词“Android”)，你将得到主要的 Android 首选项设置区域，在顶部显示 **SDK 位置**，以及下面所有安装的 Android SDK API 级别。
