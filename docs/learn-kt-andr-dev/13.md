...
最后一条语句中的`a.extract()`可以同时计算为`Any`和`String`类型，因为`b`和现在的`a`可以包含`Int`对象，但是`a`不允许包含`Int`对象，因为它只能处理`String`元素。因此科特林也不允许`a = b`。
...
在上一节中，我们看到对于`out`样式变化，相应的类不允许使用泛型类型作为函数参数，对于`in`样式变化，我们相应地不能使用返回泛型类型的函数。当然，如果我们在一个类中需要两种功能，这是不令人满意的。科特林也有这类需求的答案。它被称为*型投影*，因为它的目标是在使用一个类的不同函数时的方差，所以它是使用方方差的 Kotlin 等价物。
