...
Dokka 是 Kotlin 可以用来创建这种 API 文档的工具。要安装 Dokka，请打开 Android Studio。在 Gradle Scripts 抽屉里(你可能需要切换回 Android 视图类型)，有两个名为`build.gradle`的文件，一个标记为 Project NumberGuess，一个标记为 Module: app(见图 [6-5](#Fig5) )。这两个构建文件负责描述如何构建应用以正确运行。这包括声明需要对您的程序可用的库。
