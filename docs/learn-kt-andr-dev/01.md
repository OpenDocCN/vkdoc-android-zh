# 1.您的第一个 Kotlin 应用:Hello Kotlin
...
现在是安装设备模拟器的时候了。模拟器非常方便，因为它们允许开发 Android 应用，而无需连接真实的设备。模拟器在电脑屏幕上模拟 Android 设备。要安装一个可用的，去工具➤ AVD 管理器。出现的屏幕显示标题您的虚拟设备。单击创建虚拟设备。以下屏幕显示设备列表，如图 [1-4](#Fig4) 所示。
...
回到 Android Studio 主窗口，在应用的左侧，通过点击名称旁边的小三角形，您可以导航到以下文件(参见图 [1-6](#Fig6) ):
