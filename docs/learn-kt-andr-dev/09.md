...
一组不同的构造函数允许我们设置单独的数组成员。您所要做的就是将一个函数作为第二个参数添加到任何一个构造函数中。但是，函数怎么可能是构造函数的参数呢？这是 Kotlin 和其他计算机语言必须提供的特性之一:将函数视为可以在函数调用中传递的对象，甚至让属性指向它们。我们将在第十二章中详细介绍这些方面。我们目前需要的是一个没有函数名的所谓的*λ函数*。这样的 lambda 函数看起来像这样:
...
它们都返回一个`Pair`实例列表，如图 9-3 所示。请注意，`Iterable`是数组、集合和范围实现的接口，所以您也可以在这里使用范围。
...
![$$ stddev\left( trie s\right)=\sqrt{\frac{\sum_i^N{\left( trie{s}_i-\overline{tries}\right)}²}{N-1}} $$](img/476388_1_En_9_Chapter_TeX_Equa.png)
...
*   `Math.pow(x, 2.0)`计算平方:*x*2。

*   类似于平均值计算，`map()`提取![$$ {\left( tries-\overline{tries}\right)}² $$](img/476388_1_En_9_Chapter_TeX_IEq3.png)，其中![$$ \overline{tries} $$](img/476388_1_En_9_Chapter_TeX_IEq4.png)指定平均值。
