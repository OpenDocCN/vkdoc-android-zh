...
没有严格禁止应用向自己的组件提供数据或使用自己的数据提供者来访问内容；然而，在考虑内容提供商时，您通常会想到应用间的数据交换。但是如果你需要的话，你总是可以把应用内的数据交换模式看作是应用间通信的一个简单的特例。
...
| `grantUriPermissions` | 这指定是否可以临时授予其他应用通过 URI 访问此提供商的内容的权限。*临时授予*是指由`<permission>`、`<readPermission>`或`writePermission`定义的许可拒绝，如果内容访问客户端被带有`intent.addFlags(Intent.FLAG_GRANT_*_URI_PERMISSION)`的意图调用，则该许可被临时覆盖。如果将该属性设置为`false`，仍然可以通过设置一个或多个`<grant-uri-permission>`子元素来授予更细粒度的临时权限。默认为`false`。 |
...
在被调用组件的意图过滤器中，您必须指定一个`<data>`元素，并且它必须包含一个适当的 URI *和一个 MIME 类型*。尽管我们没有明确说明，但是必须指定 MIME 类型的原因是，Android 操作系统使用内容提供商的`getType(Uri)`方法来自动添加 MIME 类型，同时解析意图。这里有一个例子:
...
由于 URI 是如此通用的构造，并且猜测正确的 URIs 来访问某些应用提供的内容几乎是不可能的，所以内容提供商应用通常会提供一个契约类来帮助为手头的任务构建正确的 URIs。
...
然后，您可以向任何可能使用您的内容提供商应用的客户端公开提供这个契约类(或者多个类，如果您希望使用 Kotlin 和 Java 来记录接口的话)。
...
Android 操作系统及其预装的应用提供了几个内容提供商组件。在在线 API 文档中，您可以在“android.provider/Classes”部分找到内容提供者契约类。以下部分总结了它们是什么以及如何访问它们。
...
这是指允许您管理和使用基于词典的预测输入的内容提供商。从 API level 23 开始，用户词典只能在输入法编辑器或拼写检查框架中使用。对于现代应用，你不应该试图从另一个地方使用它。因此，该合同仅起信息作用。
