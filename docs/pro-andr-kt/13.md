...
表 13-2 描述了对电视搜索建议很重要的搜索项字段；左栏列出了来自`SearchManager`类的常量名称。您可以在您的数据库中使用它们，或者至少您必须在应用中提供一个映射机制。
...
检查该许可，如果获得许可，请参见第七章。如果你想将图片保存到一个公开可用的商店，以便其他应用可以看到它，你还需要以同样的方式声明和获取权限`android.permission.WRITE_EXTERNAL_STORAGE`。相反，要将图片数据保存到应用的私有空间，您需要声明一个稍微不同的权限，如下所示:
...
你从`onSensorChanged()`内的`SensorEvent.values`获得的所有可能的传感器值都列在表 13-3 中。
