...
然而，如果您的应用的类包含方法调用，并且您可以在给定各种输入集的情况下预测调用结果，那么使用标准单元测试是有意义的。将单元测试添加到您的应用中很容易。事实上，如果你使用 Android Studio 开始一个新项目，单元测试已经为你设置好了，你甚至会得到一个样本测试类，如图 [14-1](#Fig1) 所示。
