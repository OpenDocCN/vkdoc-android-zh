...
当创建您自己的小部件时，您应该覆盖`debugFillProperties()`方法来添加定制的诊断属性。这些属性可以在 Flutter 检查器中查看。在清单 [14-14](#PC15) 中，`DebugWidget`具有`name`和`price`属性。在`debugFillProperties()`方法中，使用`DiagnosticPropertiesBuilder`对象添加了两个`DiagnosticsProperty`对象。
