...
图 9-1 显示了在`debugTouchZone`和`debugCollisionZone`没有设置为`false`的情况下 app 运行；在图 9.2 中，它们被设置为`true`。
...
构造函数采用标记的名称；其纬度、经度和高度为`PhysicalLocation`；和颜色作为参数，然后将它们传递给`set()`方法。`set()`方法将这些值设置为在清单 9-56 中描述和给出的变量。它还处理摄像机的一些基本初始化和屏幕上标记的位置。
