...
任何使用`.adjustable`可访问性特征的视图都必须覆盖这两个函数。当用户的辅助技术执行递增或递减操作时，将调用这些函数。这可以是点击开关控制上的增量开关(图 [6-8](#Fig8) )或者用 VoiceOver 垂直滑动。然而，iOS 将`UISlider`控件呈现给可访问性的方式存在问题，所以让我们来看看如何解决这个问题。无论何时使用`UISlider`控件，你都应该在自己的应用中做类似的事情。如果你选择这样做的话，这也会给你知识来创建你自己的可访问的可调节控件。一般来说，我会一直推荐使用`UIKit`提供的控件。
...
当你使用辅助技术导航时，你会注意到 iOS 中经常使用的语义视图。从 iOS 的“文件”应用中查看图 [6-9](#Fig9) 中的表格单元格。这里有几条信息:文件名、日期、大小和下载按钮。如果这些是单独呈现的，那就要用画外音扫四下。如果我们激活了下载按钮，将不会清楚哪个文件会被下载。相反，您可以看到 VoiceOver 将单元格高亮显示为一个元素。
