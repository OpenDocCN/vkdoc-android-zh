...
`ButtonExample`应用程序的 redux 在`res/layout/activity_main.xml`中的正常默认位置有它的布局，尽管这种技术不管活动 XML 文件的定制命名如何都能工作。要完成从代码到布局的连接，像这样调用`setContentView()`:
