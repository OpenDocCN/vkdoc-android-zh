...
如果我们想要使用基于素材的方法，我们同样需要为我们的应用创建素材文件夹。要在 Android Studio 中创建一个 assets 文件夹，请在您的项目层次结构中突出显示 app 父级文件夹，然后在 Android Studio 中选择`File` ➤ `New` ➤ `Folder` ➤ `Assets Folder`菜单选项，这将提示为您的项目创建一个 assets 文件夹，如图 [13-1](#Fig1) 所示。
