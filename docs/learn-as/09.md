...
Git 已经成为 Android 应用开发不可或缺的工具，这向您展示了如何为您的 Android 项目初始化 Git 存储库。关于如何使用 Git 的更全面的教程，请参见第 7 章。选择 VCS ➤导入到版本控制➤创建 Git 资源库，如图 [9-7](#Fig7) 所示。当提示选择将创建新 Git 存储库的目录时，确保 Git 项目将在根项目目录中初始化，该目录名为`Currencies`，在本例中位于`C:\androidBook\Currencies`，如图 [9-8](#Fig8) 所示。如果你运行的是 Mac 电脑，在你的 Lab 父目录中选择`Currencies`目录。单击确定。
