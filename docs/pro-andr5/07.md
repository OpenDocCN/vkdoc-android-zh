...
以编程方式设计一个 TextView 控件的内容需要一点额外的工作，但是允许更多的灵活性(见[清单 7-1](#list1) )，因为你可以在运行时设计它。不过，这种灵活性只能应用于 spannable，这就是 EditText 通常管理内部文本的方式，而 TextView 通常不使用 Spannable 。span able 基本上是一个可以应用样式的字符串。要让 TextView 将文本存储为 spannable，可以这样调用 setText :
