...
对于上一章的藤蔓绘制应用，我们将使用如图 [3-7](#Fig7) 所示的一组图标。
...
选择此选项后，处理将要求创建一个新的密钥库来保存发布密钥，以便对应用包进行签名。密钥库需要密码和关于密钥库发行者的附加信息(名称、组织、城市、州、国家)，尽管这些是可选的。允许我们输入所有这些信息的密钥库管理器窗口显示在图 [3-9](#Fig9) 中。
...
如果我们手动或通过 Play Store 安装最终的应用包，我们应该会在应用启动器中看到我们为其创建的图标(图 [3-11](#Fig11) )。
