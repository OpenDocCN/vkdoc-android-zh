...
![A432415_1_En_8_Fig1_HTML.jpg](A432415_1_En_8_Fig1_HTML.jpg)
...
检查抖动的条件是`1.2 * SensorManager.GRAVITY_EARTH < accel.mag())`。这里，我们使用 1.2 作为抖动检测的阈值，我们可以使用更小或更大的值来检测更弱或更强的抖动。Android 中传感器 API 的`SensorManager`类变得很方便，因为它包含一个常数`GRAVITY_EARTH`，代表地球的重力加速度(太阳系中所有行星都有类似的常数，加上月球、太阳和虚构的死星)。时间条件已经到位，所以我们的应用程序不能每 250 毫秒触发一次以上的震动。
...
Ketai 有另一个函数`onStepCounterEvent(float s)`，我们在变量`s`中接收设备重启后的总步数。如果我们需要在应用程序未运行时跟踪一天的总步数，而又不错过活动，这可能会很有用。
...
![A432415_1_En_8_Fig2_HTML.jpg](A432415_1_En_8_Fig2_HTML.jpg)
...
注意点的最小和最大尺寸现在是如何被`displayDensity`缩放的，所以我们草图的输出保持了它的比例，而不管屏幕设备的 DPI。我们可以运行这个草图作为一个普通的应用程序或动态壁纸，以防我们想让它一直运行，并在我们的主屏幕上驱动背景图像。
...
![A432415_1_En_8_Fig3_HTML.jpg](A432415_1_En_8_Fig3_HTML.jpg)
...
磁性传感器(或磁力计)是我们在 Android 设备中发现的另一种非常常见的传感器，它对几个应用程序都很有用。例如，清单 [8-7](#Par32) 显示了我们如何使用它来检测金属物体的接近程度，方法是将磁场的测量值与我们当前位置的地球磁场的预期值进行比较。如果我们在手机上运行这个草图，我们将有效地把它变成一个金属探测器！
...
![A432415_1_En_8_Fig4_HTML.jpg](A432415_1_En_8_Fig4_HTML.jpg)
...
![A432415_1_En_8_Fig5_HTML.jpg](A432415_1_En_8_Fig5_HTML.jpg)
...
![A432415_1_En_8_Fig6_HTML.jpg](A432415_1_En_8_Fig6_HTML.jpg)
...
使用陀螺仪时，我们不仅限于处理 3D 几何图形。如果我们在 2D 绘图，我们需要做的就是跟踪 z 旋转，如清单 [8-12](#Par50) 所示。
...
陀螺仪可用于在游戏应用程序中实现输入。我们将在本章的最后一节看到如何做到这一点。
...
![A432415_1_En_8_Fig7_HTML.jpg](A432415_1_En_8_Fig7_HTML.jpg)
...
![A432415_1_En_8_Fig8_HTML.jpg](A432415_1_En_8_Fig8_HTML.jpg)
