...
在介绍更高级的壁纸示例之前，我们将通过一个显示背景图像的简单应用来了解动态壁纸的一些重要特性。我们已经看到了如何在处理中加载和显示图像。同样的方法也适用于壁纸:只需将一个图像文件复制到草图的数据文件夹，用`loadImage()`加载，用`image()`显示，这样它就覆盖了设备的整个屏幕，如清单 [6-2](#Par10) 所示。
...
为了让这种方法正常工作，我们需要提供一个驱动粒子运动的速度流场。我们认为触摸互动是这种运动的潜在来源。例如，每当指针改变位置时，我们可以计算测量指针位置变化的向量，即`(mouseX - pmouseX, mouseY – pmouseY)`，作为位置`(mouseX, mouseY)`处的“速度”。让我们在清单 [6-7](#Par36) 中实现这种方法。在这个例子中，我们将使用两个类来组织代码——一个存储每个粒子，另一个保存整个场。我们可以在 Java 模式下运行它，也可以在我们的设备或模拟器上作为常规应用运行。输出应该看起来或多或少如图 [6-7](#Fig7) 所示。
