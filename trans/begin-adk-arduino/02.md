...
生成的项目不仅仅是一个骨架。它已经是一个成熟的应用，尽管非常初级。它只显示“Hello World，HelloWorldActivity”。让我们来看看这个应用的结构和生命周期。每个 Android 应用的起点是`AndroidManifest.xml`。打开它，看看它的结构([清单 2-2](#list_2_2) )。
