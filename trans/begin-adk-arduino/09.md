...
Android 部分到此为止，最终结果如图[图 9-8](#fig_9_8) 所示。当你准备好了，部署这两个应用，看看每当你倾斜你的 Android 设备时，你的伺服如何转动。
...
Android 应用与之前的伺服项目几乎相同。唯一需要改变的是消息字节和当`SensorEventListener`接收到`SensorEvent`时得到的值。让我们看看需要做些什么([清单 9-5](#list_9_5) )。
...
Android 代码这次没有太大的麻烦，因为您只需要调整上一个示例中的一些代码行。然而，你已经完成了机器人的部分，现在你可以看到你的 DC 汽车在运行了([图 9-17](#fig_9_17) )。部署 Arduino sketch 和 Android 应用，当您向前倾斜设备时，可以看到您的电机在旋转。
