...
在这一点上，你可能想参考 Android 开发者文档，并仔细阅读 Intent 主题。理解如何使用意图来启动一个活动将会让你洞察到你需要为你的应用提供什么。虽然您可以使用 ADB 实用程序从命令行启动几乎任何 Android 活动，但是您也可以以编程方式启动活动。后面的章节将使用这一概念来构建脚本以自动化多个活动。
