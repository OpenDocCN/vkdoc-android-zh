...
到目前为止，我们已经讨论了静态菜单——你设置了一次，它们不会根据屏幕上的内容动态变化。如果你想创建动态菜单，使用 Android 在活动类上提供的 onPrepareOptionsMenu() 方法。这个方法类似于 onCreateOptionsMenu() ，除了它在每次显示菜单之前被调用。如果你的菜单有动态菜单选项，你应该使用 onPrepareOptionsMenu() 和 onCreateOptionsMenu() 来有效地管理你的菜单。 onPrepareOptionMenu() 是根据您正在显示的内容启用或禁用某些菜单项或菜单组的地方。对于 3.0 及更高版本，当您想要更改菜单时，因为像操作栏这样的菜单相关组件总是显示，所以您必须显式调用一个名为activity . invalidateoptions menu()的新预配方法，该方法又调用 onCreateOptionsMenu() 并重新绘制菜单，从而也导致在显示之前调用 onPrepareOptionsMenu() 。只要应用状态发生变化，需要更改菜单，就可以调用此方法。
