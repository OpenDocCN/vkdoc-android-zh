...
我们将在示例应用中使用 search 来演示加载器的动态特性。我们在菜单上附加了一个搜索视图。您可以在[清单 26-2](#list2) 中的方法onCreateOptionsMenu()中看到这一点。这里，我们将一个 SearchView 附加到菜单上，并在 SearchView 中提供新文本时，将该活动作为对 SearchView 的回调。在[清单 26-2](#list2) 的方法onQueryTextchange()中处理 SearchView 回调。
