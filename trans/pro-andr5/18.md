...
在[清单 18-8](#list8) 中，android:layoutAnimation 是指向[清单 18-7](#list7) 的中介 XML 文件的标签，该文件又指向[清单 18-5](#list5) 的 scale.xml 。在[清单 18-8](#list8) 中，Android SDK 文档建议在列表视图上设置 persistentDrawingCache 标签，以优化动画和滚动。如果您要运行应用pro Android 5 _ ch18 _ testlayoutanimation . zip，您将会看到当加载活动时，缩放动画在单个列表项上生效。我们已经将动画持续时间设置为 500 ms，以便在绘制每个列表项时可以观察到比例变化。
...
有了这段代码，你会看到用户界面如图 18-4 所示。为了给[图 18-4](#Fig4) 所示的列表视图添加动画，我们需要一个从Android . view . animation . animation派生的类。清单 18-17 显示了这个类。
...
[***清单 18-19***](#_list19) 。使用预翻译和后翻译T10】查看动画
...
在[清单 18-23](#list23) 中的viewoanimationlistener类只是记录消息。[清单 18-24](#list24) 中的代码展示了如何将一个动画监听器附加到一个动画对象。
